...
int main(int argc, char **argv)
{
    ...
    double tag_1_put_y = 2.28; // 放置tag1的y坐标

    double tag_2_put_x = 1.06;   // 放置tag2的x坐标
    double tag_2_put_y = 2.28; // 放置tag2的y坐标
    ...
    //导航到货架
    sendGoal(ac, grab_desk_x, grab_desk_y + offset_left, 0);
    ...
    if (tag_id == 1)
    {
        ...
        // 识别
        system("roslaunch upros_cv face_rec.launch");
        
        // 获取人脸识别结果并输出
        std::string recognized_face;
        double confidence;
        nh.getParam("/recognized_face", recognized_face);
        nh.getParam("/recognition_confidence", confidence);
        ROS_INFO("result: %s, confidence: %.2f", recognized_face.c_str(), confidence);

        if (recognized_face == "man") {
            system("roslaunch carry_robot arm_put_left.launch");
        } else if (recognized_face == "woman") {
            system("roslaunch carry_robot arm_put_right.launch");
        } else {
            ROS_INFO("none");
        }
    }
